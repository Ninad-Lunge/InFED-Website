...
const EventsCalendar = () => {
  ...
  return (
    <div className="relative w-full max-w-6xl mx-auto px-4 py-8">
      ...
      <div className="relative">
        {events.map((event, index) => {
          ...
          return (
            <div
              key={event.id}
              className={`flex ${isLeft ? 'justify-end mr-auto' : 'justify-start ml-auto'} w-1/2 mb-8`}
            >
              {/* Timeline dot */}
              <div className="absolute left-1/2 transform -translate-x-1/2 w-4 h-4 rounded-full bg-white border-4 border-gray-300 z-10" 
                   style={{ borderColor: monthColor }} />
              
              {/* Card */}
              <div
                className={`flex flex-col w-[500px] h-[240px] rounded-xl shadow-lg relative ${
                  isLeft ? 'mr-8' : 'ml-8'
                }`}
                style={{
                  borderWidth: '1px',
                ...
                <div className="flex flex-row p-5">
                  <div>
                    ...
                    <div className="flex flex-row">
                      ...
                      />
                      <h4 className="text-gray-600 text-left mr-auto">{event.type}</h4>
                    </div>
                  </div>
      ...
      >
        <ArrowDownCircle className="animate-bounce text-gray-400" size={32} />
      </div>
    </div>
  );
};
